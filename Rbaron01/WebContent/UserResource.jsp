...
<div class="flex-conteiner">
		...
		if(request.getParameter("meta") != null){
			...
			if(meta == 0){
				...
				ClientOperation.deleteRis(false, id);
			}else{
				int id = Integer.parseInt(request.getParameter("id_meta"));
				ClientOperation.deleteRis(true, id);
			}
		}
		%>
		
		<div class="flex-conteiner">
		<% ArrayList<SingleRow> ris = ClientOperation.userResources(false).result();
		...
		%>
		<form action='UserResource.jsp'>
			<input type='submit' value='supprime'>
			<input  type='hidden' align='center' value='<% out.print(ris.get(i).get().get("id_meta")); %>' readonly='readonly' name='id_book'>
	...
	<br>
		...
		<%  ris = ClientOperation.userResources(true).result();
			for(int i=0;i<ris.size();i++){
				out.println("<div class='flex-item'>");
